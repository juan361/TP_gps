...
const char *findChar(const char *str, const char c, int pos) {
    ...
    for (int index = pos; i <
                          strlen(str); index++) {        //Début d'une boucle 'pour' avec un index à remplir qu'on incrémente a chaque fois
...
const char *findStr(const char *str, const char *s, int pos) {
    ...
    while (warning !=
           0) {                        //Début d'une boucle 'tant que' warning est pas égal à 0 on fait notre boucle
        ...
        for (int index = 0; index <
                            strlen(s); index++) {        //Début d'une boucle 'pour' dans la qu'elle on déclare un entier index qui est égal à zéro qu'on incrémente à chaque fois qu'on 									utilise la boucle
            if (*(poslettre + index) ==
                s[index]) {            //Début de la boucle 'si' dans la qu'elle on additionne poslettre et index qu'on compare avec s[index]
                warning--;
            }                            //Fin de la boucle 'si'
        }                                //Fin de la boucle 'pour'
...
int isValidGpsData(const char *str_signal) {
    ...
    int entete;
    entete = isValidHeader(
    const char *str_signal);
    return entete
    if entete == UNKNOWN_FRAME;
...
strcpy( buffer, getField(gotoField( frame,
5)));
// degres/minute => minute on divise par 60
